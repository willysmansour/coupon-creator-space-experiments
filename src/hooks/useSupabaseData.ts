...
export const useCoupon = (id: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // ✅ Fix: Use Edge Function for mobile, direct Supabase for desktop
      if (networkInfo.isMobile) {
        console.log('🔍 Using Edge Function for mobile');
        
        const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-coupon`, {
          method: 'POST',
          headers: {
        ...
        console.log('🔍 Using direct Supabase for desktop');
